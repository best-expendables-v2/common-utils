...
func ParseValidationErr(err error) []ValidationError {
	...
	for _, fieldError := range err.(validator.ValidationErrors) {
		...
		message := buildFieldValidationErrorMessage(fieldError, fieldName)
		code := fieldError.Tag()
		argument := fieldError.Param()
		errs = append(errs, ValidationError{Field: fieldName, Message: message, Code: code, Argument: argument})
	}
